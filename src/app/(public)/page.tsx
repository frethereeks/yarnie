...
export default async function Home() {
  const {data: res} = await fetchHomeData({showHidden: true, limit: 8})
  ...
  return (
    <>
      <main className="flex flex-col bg-background">
        <section className='bg-white px-4'>
          <div className="container mx-auto py-10 lg:my-10 lg:px-10 flex flex-col md:flex-row gap-4 lg:justify-center">
            <aside className="flex-1 relative z-10 flex flex-col justify-center gap-4 py-5 md:py-10 md:px-4">
              <h1 className="text-5xl md:text-5xl text-secondary font-bold font-capriola lg:max-w-md">Hand-hooked goods made with love.</h1>
              <p className="font-play text-text text-base text-justify leading-loose lg:max-w-md">The Yarnie is all about a crochet making, crochet making is the process of creating fabric or other similar items using a crochet hook to interlock loops of yarn, thread, or other materials. It&apos;s a versatile craft that can be used to make a wide variety of items, from clothing and accessories to home décor and toys.</p>
              {/* <p className="font-play text-text text-base text-justify lg:max-w-md">Explore the shop for more gift ideas.</p> */}
            ...
            <aside className="flex-1 relative bg-transparent rounded-xl min-h-60 row-start-1 md:row-span-2 grid place-items-center">
              <div className="h-60 w-60 md:w-80 md:h-80 mx-auto grid place-items-center rounded-full relative before:absolute before:w-[120%] before:h-[120%] before:left-1/2 before:top-1/2 before:-translate-x-1/2 before:-translate-y-1/2 before:rounded-full before:bg-secondary/50">
        ...
        <section className="px-4 bg-white text-white">
          <div className="container mx-auto flex flex-col gap-4 md:gap-8 py-10 md:py-20">
            {
              yarnData.slice(0,2).map((yarn,i) => (
                ...
                <div className="flex-1 relative z-10 flex flex-col justify-center gap-4 py-5 md:py-10 md:px-4">
                  <h4 className="text-3xl md:text-3xl text-secondary text-center md:text-justify font-bold font-capriola lg:max-w-md">{yarn.name}</h4>
                  <p className="font-play text-text text-base text-center md:text-justify leading-loose lg:max-w-md">{yarn.description}</p>
                </div>
        ...
        <section className="relative py-20 md:py-40 px-4 bg-primary">
          ...
          <div className="relative container mx-auto flex flex-col justify-center items-center gap-4 md:gap-6">
            <h4 className="text-2xl md:text-3xl text-center text-white font-bold font-capriola w-full max-w-xl">We deliver the best hand-stitched yarn products you would ever see!</h4>
